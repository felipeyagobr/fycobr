...
// Busca a logo no banco de dados
$stmt_logo = $pdo->query("SELECT logo FROM config LIMIT 1");
$config = $stmt_logo->fetch(PDO::FETCH_ASSOC);
$logo_path = $config['logo'] ?? '/assets/img/default-logo.png'; // Caminho para uma logo padrão caso não encontre
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO usuarios (nome, telefone, email, senha, saldo, indicacao, banido, comissao_cpa, comissao_revshare, created_at, updated_at) VALUES (?, ?, ?, ?, 0, ?, 0, ?, ?, NOW(), NOW())");

    try {
        // 1. PRIMEIRO: Executa a inserção do usuário
        $stmt->execute([$nome, $telefone, $email, $senha, $ref, $cpa_padrao, $revshare_padrao]);

        // 2. SEGUNDO: Pega o ID do usuário que acabou de ser criado
        $usuarioId = $pdo->lastInsertId();

        // --- INÍCIO DA MODIFICAÇÃO ---

        // Tenta notificar o n8n sobre o novo cadastro.
        try {
            $client = new \GuzzleHttp\Client();

            // IMPORTANTE: Substitua pela URL do seu Webhook do n8n que criaremos no próximo passo.
            $n8nWebhookUrl = 'https://webhook.felipeyago.com/webhook/cad-raspadinhabr.bet';
            ...
            $client->post($n8nWebhookUrl, [
                'json' => [
                    ...
                    'email' => $email,
                    'telefone' => $telefone,
                    'data_cadastro' => (new DateTime('now', new DateTimeZone('America/Sao_Paulo')))->format('Y-m-d')
                ],
                // Garante que o script não espere muito por uma resposta do n8n
                'timeout' => 2,
            ]);
        } catch (\Exception $e) {
            // Se a chamada para o n8n falhar, apenas registre o erro no log do PHP.
            // O cadastro do usuário não será afetado.
            error_log('Erro ao chamar o webhook do n8n: ' . $e->getMessage());
        }

        // --- FIM DA MODIFICAÇÃO ---

        // 3. TERCEIRO: Lógica do Avatar, agora com o ID correto
        try {
            $avatarPath = $_SERVER['DOCUMENT_ROOT'] . '/assets/avatar/';
            $avatars = glob($avatarPath . '*.{jpg,jpeg,png,gif}', GLOB_BRACE);

            if (!empty($avatars)) {
                $randomAvatar = $avatars[array_rand($avatars)];
                $relativeAvatarPath = str_replace($_SERVER['DOCUMENT_ROOT'] . '/', '', $randomAvatar); // Salva o caminho relativo

                $stmtAvatar = $pdo->prepare("UPDATE usuarios SET avatar = :avatar WHERE id = :id");
                $stmtAvatar->execute(['avatar' => $relativeAvatarPath, 'id' => $usuarioId]);
            }
        } catch (Exception $e) {
            // Se o avatar der erro, não impede o cadastro, apenas registra.
            error_log("Erro ao processar avatar para o usuário ID $usuarioId: " . $e->getMessage());
        }

        // 4. QUARTO: Finaliza o processo
        ...
        $_SESSION['message'] = ['type' => 'failure', 'text' => 'Erro ao realizar cadastro! E-mail já pode estar em uso.'];
...
<body>
                ...
                <div class="cadastro-card">
                    <div class="cadastro-header">
                        <div class="cadastro-icon">
                            <img src="<?php echo $logo_path; ?>" alt="Logo" style="width: 100%; height: auto;">
                        </div>
    ...
    <script>
        ...
        console.log('%c🎯 RaspadinhaBR - Cadastro Split Screen', 'color: #78d403; font-size: 16px; font-weight: bold;');
    </script>
</body>
