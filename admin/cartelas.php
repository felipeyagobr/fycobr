...
// LÓGICA PARA ATIVAR/DESATIVAR DESTAQUE
if (isset($_GET['toggle_destaque']) && isset($_GET['id'])) {
    $raspadinha_id = (int)$_GET['id'];

    // Primeiro, busca o status atual
    $stmt = $pdo->prepare("SELECT destaque FROM raspadinhas WHERE id = ?");
    $stmt->execute([$raspadinha_id]);
    $raspadinha = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($raspadinha) {
        // Inverte o status atual (se for 1, vira 0; se for 0, vira 1)
        $novo_status = $raspadinha['destaque'] == 1 ? 0 : 1;

        $updateStmt = $pdo->prepare("UPDATE raspadinhas SET destaque = ? WHERE id = ?");
        $updateStmt->execute([$novo_status, $raspadinha_id]);

        $_SESSION['message'] = ['type' => 'success', 'text' => 'Status de destaque alterado com sucesso!'];
    } else {
        $_SESSION['message'] = ['type' => 'failure', 'text' => 'Raspadinha não encontrada.'];
    }

    // Redireciona para limpar os parâmetros da URL
    header("Location: cartelas.php");
    exit;
}
...
if (isset($_POST['adicionar_raspadinha'])) {
    ...
    $chance_de_vitoria = str_replace(',', '.', $_POST['chance_de_vitoria']);
    ...
    $stmt = $pdo->prepare("INSERT INTO raspadinhas (nome, descricao, banner, valor, chance_de_vitoria) VALUES (?, ?, ?, ?, ?)");
    if ($stmt->execute([$nome, $descricao, $banner, $valor, $chance_de_vitoria])) {
...
if (isset($_POST['editar_raspadinha'])) {
    ...
    $stmt = $pdo->prepare("UPDATE raspadinhas SET nome = ?, descricao = ?, banner = ?, valor = ?, chance_de_vitoria = ? WHERE id = ?");
    if ($stmt->execute([$nome, $descricao, $banner, $valor, $chance_de_vitoria, $id])) {
...
<head>
    ...
    <style>
        * {
        ...
        /* Remove o fundo antigo do hover e do active */
        .nav-item:hover,
        .nav-item.active {
            color: #ffffff;
        ...
        /* Faz a barrinha aparecer no hover ou quando o item está ativo */
        ...
        .content-grid {
            ...
            gap: 2rem;
            /* Aumentar de 1.5rem para 2rem ou mais */
        }
                ...
                <section class="form-section">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="chance_de_vitoria" value="<?= $raspadinha_edit ? htmlspecialchars(number_format($raspadinha_edit['chance_de_vitoria'], 2, ',', '.')) : '25,00' ?>" class="form-input" placeholder="25,00" required>
                        </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($raspadinhas as $raspadinha): ?>
                                <div class="raspadinha-card">
                                    ...
                                    <div class="raspadinha-date" style="color: #22c55e">
                                        <i class="fas fa-percentage"></i>
                                        <span>Chance de Vitória: <?= number_format($raspadinha['chance_de_vitoria'], 2, ',', '.') ?>%</span>
                                    </div>
                                    <div class="raspadinha-date" style="margin-top: 1rem;">
                                        <i class="fas fa-star" style="color: <?= $raspadinha['destaque'] == 1 ? '#FFD700' : '#6b7280' ?>;"></i>
                                        <span>Destaque: <strong><?= $raspadinha['destaque'] == 1 ? 'Sim' : 'Não' ?></strong></span>
                                    </div>
                                    ...
                                    <div class="action-buttons">
                                        ...
                                        <?php if ($raspadinha['destaque'] == 1): ?>
                                            <a href="?toggle_destaque=1&id=<?= $raspadinha['id']; ?>" class="action-btn" style="background: linear-gradient(135deg, #f59e0b, #d97706); color: white;">
                                                <i class="fas fa-star-slash"></i>
                                                Desativar Destaque
                                            </a>
                                        <?php else: ?>
                                            <a href="?toggle_destaque=1&id=<?= $raspadinha['id']; ?>" class="action-btn" style="background: linear-gradient(135deg, #10b981, #059669); color: white;">
