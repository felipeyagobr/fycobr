...
if (empty($data['amount'])) {
    echo json_encode(['success' => false, 'message' => 'O valor do saque é obrigatório.']);
    exit;
}
...
if ($amount < $saqueMin) {
    echo json_encode(['success' => false, 'message' => 'O valor do saque é menor que o mínimo permitido de R$ ' . number_format($saqueMin, 2, ',', '.')]);
    exit;
}
...
try {
    ...
    $stmt = $pdo->prepare("SELECT saldo, cpf, rollover_pending FROM usuarios WHERE id = :id FOR UPDATE");
    $stmt->bindParam(':id', $usuario_id, PDO::PARAM_INT);
    $stmt->execute();
    $usuario = $stmt->fetch(PDO::FETCH_ASSOC);
// --- INÍCIO DA VERIFICAÇÃO DE ROLLOVER ---
if ($usuario['rollover_pending'] > 0) {
    // Joga uma exceção que será capturada pelo bloco catch no final do arquivo
    throw new Exception('Você possui um rollover de R$ ' . number_format($usuario['rollover_pending'], 2, ',', '.') . ' pendente. Cumpra o rollover para poder sacar.');
}
// --- FIM DA VERIFICAÇÃO DE ROLLOVER ---

    if (empty($usuario['cpf'])) {
        echo json_encode(['success' => false, 'message' => 'Você precisa cadastrar seu CPF no perfil antes de solicitar um saque.']);
        exit;
    }
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL => "https://api-cpf-gratis.p.rapidapi.com/?cpf=" . $usuario['cpf'],
